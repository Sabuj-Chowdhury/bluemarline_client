    ...
    >
      <div className="container mx-auto px-4 md:px-8 max-w-7xl relative flex items-center justify-between z-50">
        ...
        <Link to="/" className="flex items-center gap-3 z-50 group">
          ...
          <span
            className={`text-2xl font-black tracking-tight transition-colors duration-500 ${scrolled ? "text-white" : isDarkHero ? "text-white" : "text-zinc-900"}`}
          >
            BlueMarlin
          </span>
        </Link>
        ...
        <nav className="hidden lg:flex items-center gap-8">
          {navLinks.map((link) => {
            ...
            const textColorInactive = scrolled
            ...
            return (
              <Link
                ...
                className={`relative text-sm font-bold tracking-wider transition-all duration-300 uppercase hover:drop-shadow-[0_0_8px_rgba(255,255,255,0.4)] ${
                  isActive
                    ? scrolled || isDarkHero
                      ? "text-white drop-shadow-[0_0_8px_rgba(37,99,235,0.8)]"
                      : "text-zinc-900"
                    : textColorInactive
                }`}
              >
        ...
        >
          {isOpen ? (
            <X size={28} className="text-white relative z-50" />
          ) : (
            <Menu size={28} />
          )}
        </button>
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {link.name}
                      {isActive && (
                        <span className="inline-block ml-3 w-2 h-2 rounded-full bg-primary align-middle shadow-[0_0_10px_rgba(37,99,235,0.8)]"></span>
                      )}
                    </Link>
                  </motion.div>
                );
